...
class NewDebter:
    def __init__(self):
        """Funkcja inicjalizująca klasę NewDebter a zarazem całą aplikację.
        Zainicjalizowane jest podłączenie bazy danych
        ktora zostala zaprogramowana przy uzyciu sqlite3 oraz graficznego
        interfejsu (tkinter)
        """
        ...
        # Przyciski do wyswietlania historii, wyswietalnia wydatkow wg kategorii
        # i wskazywanie aktualnego stanu konta
        # wykonują one w nowych wątkach funkcje
        but = tkinter.Button(self.frame, text="Historia", background="pink",
                height='2', width=10,
                command=lambda: threading.Thread(target=self.historia()))
        but.grid(row=0, column=0)
        but1 = tkinter.Button(self.frame, text="Wyświetl kategorię",
                background="pink", height=2,
                command=lambda: threading.Thread(target=self.selectByCategory()))
        but1.grid(row=0, column=1)
        self.suma = tkinter.Label(self.frame, text=self.sum,
                background="white", height=2, width=10).grid(row=0,
                        column=2)
        
        # dodawanie wydatku
        tkinter.Label(self.frame, text="Dodaj wydatek do bazy:",
                background="white", height=2).grid(row=1, column=1)
        tkinter.Label(self.frame, text="Kwota:", background="white",
                anchor="e").grid(row=2, column=0)
        self.kwota1 = tkinter.StringVar()
        tkinter.Entry(self.frame, text=self.kwota1,
                background="white").grid(row=2, column=1)
        tkinter.Label(self.frame, text="Kategoria:", background="white"
                ).grid(row=3, column=0)
        #takie dziwne cos
        self.box_value = tkinter.StringVar()
        self.box = ttk.Combobox(self.frame, textvariable=self.box_value,
                state='readonly')
        self.box['values'] = (
            'jedzenie', 'rozrywka', 'dlug', 'zwrot od wierzyciela', 'wyplata',
            'ksiazki', 'rozwijanie sie')
        self.box.current(0)
        self.box.grid(row=3, column=1)
        tkinter.Label(self.frame, text="Dodatkowe dane:", background="white"
                ).grid(row=4, column=0)
        self.d_info = tkinter.StringVar()
        tkinter.Entry(self.frame, text=self.d_info, background='white'
                ).grid(row=4, column=1)
        tkinter.Button(self.frame, text="Dodaj!", background="pink"
                , command=self.addRec).grid(row=4, column=2)
    ...
    def setSum(self):
        """Sprawdza czy w folderze z aplikacją znajduje się plik kwota.txt,
        ktory zawiera kwote znajdujaca sie na koncie.
        Jezeli plik takowy nie istnieje pobiera daną i zapisuje ją w pliku o
        takiej nazwie
    ...
    def changeSum(self, newsum):
        """Po dodaniu nowego rekordu do bazy poprawia ilosc pieniedzy
    ...
    def addRec(self):
        """Dodaje do bazy danych rekord postaci aktualna data; ilosc 
        pieniedzy wydanych, z jakiej kategorii oraz dodatkowe informacje. Po 
        dodaniu rekordu, wykonuje funkcję zmiany sumy pieniędzy, która 
        pozostała użytkownikowi
        """
        kwota = self.kwota1.get()
        kategoria = self.box.get()
        dinfo = self.d_info.get()
        with self.database:
            self.dbcursor.execute('insert into Money values (date("now")
                    ,?,?,?)', (kwota, kategoria, dinfo))
    ...
    def selectByCategory(self):
        """Pozwala wyselekcjonować 10 ostatnich rekordów z bazy danych,
        ktore naleza do danej kategorii.
        ...
        tkinter.Label(self.ramka, text="Ktora kategorie poszukujesz?"
                ).grid(row=0)
        #takie dziwne cos
        box_value = tkinter.StringVar()
        box = ttk.Combobox(self.ramka, textvariable=box_value, state='readonly')
        box['values'] = ('jedzenie', 'rozrywka', 'dlug', 'zwrot od wierzyciela',
                'wyplata', 'ksiazki', 'rozwijanie sie')
        box.current(0)
        box.grid(row=1, column=0)
        tkinter.Button(self.ramka, text="OK", command=lambda:
                self.showByCategory(box.get())).grid(row=1, column=1)
        self.okno.mainloop()

    def showByCategory(self, kategory):
        """rozszerzenie funkcji selectByCategory: pobiera z niej informacje 
        jaka kategoria nas interesuje a następnie
        ...
        self.dbcursor.execute("select Data,Ile,D_info from Money where 
                Kategoria=?", (kategory,))
